...
export const VehicleList: React.FC = () => {
  ...
  const handleDownloadTemplate = () => {
    const ws = XLSX.utils.json_to_sheet([
      ...
      { fleetNumber: "505", licensePlate: "CC-11-DD", type: "Elétrico", station: "Santo Amaro", model: "Remodelado" }
    ]);
                    ...
                    {vehicles.length === 0 ? (
                        ...
                        <tr key={v.fleetNumber} className="hover:bg-gray-50">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${v.type === VehicleType.TRAM ? 'bg-orange-100 text-orange-800' : 'bg-blue-100 text-blue-800'}`}>
                                    {v.type}
                                </span>
                            </td>
                    ...
                    vehicles.map((v) => (
                        <div key={v.fleetNumber} className="bg-gray-50 p-4 rounded-lg border border-gray-200 shadow-sm">
                            <div className="flex justify-between items-center mb-2">
                                ...
                                <span className={`px-2 py-1 text-xs font-bold rounded-full uppercase ${v.type === VehicleType.TRAM ? 'bg-orange-100 text-orange-800' : 'bg-blue-100 text-blue-800'}`}>
                                    {v.type}
                                </span>
                            </div>
